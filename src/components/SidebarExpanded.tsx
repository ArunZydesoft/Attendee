  ...
  return (
    <div
      className={`transition-all duration-500 h-full ${
        sidebarExpanded ? "w-[40%]" : "w-30"
      } pt-4 flex flex-col items-center bg-cover bg-center text-white relative rounded-r-lg overflow-hidden shrink-0 `}
      style={{
        ...(sidebarExpanded
          ? {}
    ...
    >
      {sidebarExpanded ? (
        <>
          ...
          <div className="relative text-left px-6 pt-10  h-full overflow-y-auto z-10">
            <button
              onClick={() => setSidebarExpanded(!sidebarExpanded)}
              className="absolute top-4 -right-1 -translate-x-1/2 w-6 h-6 bg-black text-white rounded-full flex items-end justify-center"
            >
              X
            </button>
        ...
        <>
          <button
            onClick={() => setSidebarExpanded(!sidebarExpanded)}
            className="absolute top-4 left-1/2 -translate-x-1/2 w-6 h-6 bg-black text-white rounded-full flex items-center justify-center"
          >
            →
          </button>
          <div className="mt-80 rotate-270 whitespace-nowrap text-6xl text-center">
            Intersec Exhibition
          </div>
        </>
      )}
